...
public class SmartAI : BattleshipPlayer
{
	...
	public static bool waitingForRoutine = false;
	public static List<Ship> mShips;
	public static Ship shipToPlace;
		...
		if (!shipsReady) {
			if (!waitingForRoutine) {
				if (curship < mShips.Count) {
					...
					StartCoroutine (PlaceShipManually (mShips [curship]));
	...
	public System.Collections.IEnumerator PlaceShipManually (Ship ship)
	{
		...
		if (ship != null) {
			...
			/*
		GameObject shipPlace = GameObject.CreatePrimitive(PrimitiveType.Cube);
		shipPlace.transform.Translate(new Vector3(0,-5,0));
		
		shipPlace.AddComponent("ShipPlacementGUI");
		ShipPlacementGUI.currentShip = ship;
		shipPlace.GetComponent<ShipPlacementGUI>().enabled = true;
		...
		*/
			...
			Application.LoadLevel ("ShipPlacement");
	
			Init_board.gameGrid = ShipPlacementGUI.gameGrid;
			
			Debug.Log("curship: " + curship);
			Debug.Log("mShips.count");
	...
	public System.Collections.IEnumerator PlaceShip (Ship ship)
	{
		...
		if (ship != null) {
			...
			if (!success)
				throw new OperationCanceledException ("PlaceShip");
	...
	public System.Collections.IEnumerator GetShot ()
	{
		...
		while (!success) {
			...
			if (mustExplore.Count > 0) {
				...
				// if there are no outstanding hits, explore empty space else
				shot = GetSpacePoint ();
			}
	...
	public override void OpponentHit (bool hit, bool sunk)
	{
		...
		if (sunk) {
			int left = x;
			while (left > 0 && (state[left - 1, y] == State.HIT || state[left - 1, y] == State.SUNK))
				--left;
			int right = x;
			while (right < width - 1 && (state[right + 1, y] == State.HIT || state[right + 1, y] == State.SUNK))
				++right;
			int above = y;
			while (above > 0 && (state[x, above - 1] == State.HIT || state[x, above - 1] == State.SUNK))
				--above;
			int below = y;
			while (below < height - 1 && (state[x, below + 1] == State.HIT || state[x, below + 1] == State.SUNK))
				++below;

			int sunkWidth = right - left + 1;
			int sunkHeight = below - above + 1;

			if (sunkWidth > 1 && sunkHeight > 1)
				return;
			else if (sunkWidth > 1) {
				int numShips = 0;
		...
		if (DEBUG)
			Console.WriteLine ("{0} State.HIT {1}", shot, sunk);
	}
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				if (state [x, y] == State.HIT || state [x, y] == State.SUNK) {
					int left = x;
					while (left > 0 && (state[left - 1, y] == State.UNKNOWN || state[left - 1, y] == State.HIT || state[left - 1, y] == State.SUNK))
						--left;
					int right = x;
					while (right < width - 1 && (state[right + 1, y] == State.UNKNOWN || state[right + 1, y] == State.HIT || state[right + 1, y] == State.SUNK))
						++right;
					int above = y;
					while (above > 0 && (state[x, above - 1] == State.UNKNOWN || state[x, above - 1] == State.HIT || state[x, above - 1] == State.SUNK))
						--above;
					int below = y;
					while (below < height - 1 && (state[x, below + 1] == State.UNKNOWN || state[x, below + 1] == State.HIT || state[x, below + 1] == State.SUNK))
						++below;
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				if (state [x, y] == State.HIT) {
					int left = x;
					while (left > 0 && state[left - 1, y] == State.HIT)
						--left;
					if (left > 0 && state [left - 1, y] == State.UNKNOWN)
						continue; // not bounded
					int right = x;
					while (right < width - 1 && state[right + 1, y] == State.HIT)
						++right;
					if (right < width - 1 && state [right + 1, y] == State.UNKNOWN)
						continue;
					int above = y;
					while (above > 0 && state[x, above - 1] == State.HIT)
						--above;
					if (above > 0 && state [x, above - 1] == State.UNKNOWN)
						continue;
					int below = y;
					while (below < height - 1 && state[x, below + 1] == State.HIT)
						++below;
					if (below < height - 1 && state [x, below + 1] == State.UNKNOWN)
						continue;
			...
			for (int x = 0; x < width; ++x) {
				if (state [x, y] == State.HIT) {
					foreach (int w in remainingShips) {
						...
						while (left > endl && (state[left - 1, y] == State.UNKNOWN || state[left - 1, y] == State.HIT || state[left - 1, y] == State.SUNK))
							--left;
						int right = x, endr = Math.Min (x + (w - 1), width - 1);
						while (right < endr && (state[right + 1, y] == State.UNKNOWN || state[right + 1, y] == State.HIT || state[right + 1, y] == State.SUNK))
							++right;
						if (right - left + 1 >= w) {
							for (int i = left; i <= right; ++i) {
								if (state [i, y] == State.UNKNOWN) {
						...
						while (above > enda && (state[x, above - 1] == State.UNKNOWN || state[x, above - 1] == State.HIT || state[x, above - 1] == State.SUNK))
							--above;
						int below = y, endb = Math.Min (y + (w - 1), height - 1);
						while (below < endb && (state[x, below + 1] == State.UNKNOWN || state[x, below + 1] == State.HIT || state[x, below + 1] == State.SUNK))
							++below;
						if (below - above + 1 >= w) {
							for (int i = above; i <= below; ++i) {
								if (state [x, i] == State.UNKNOWN) {
	...
	{
		if (state [x, y] != State.UNKNOWN)
			left [x, y] = right [x, y] = below [x, y] = above [x, y] = 0;
		else {
			// left
			if (x == 0 || state [x - 1, y] != State.UNKNOWN)
				left [x, y] = 0;
			else
			...
			// above
			if (y == 0 || state [x, y - 1] != State.UNKNOWN)
				above [x, y] = 0;
			else
				above [x, y] = above [x, y - 1] + 1;
			// below
			if (y == 0 || below [x, y - 1] == 0) {
				int ctr = 0;
				for (int yp = y + 1; yp < height; ++yp) {
					if (state [x, yp] != State.UNKNOWN)
						break;
					++ctr;
				}
				below [x, y] = ctr;
			} else
				below [x, y] = below [x, y - 1] - 1;
		}
	}
	...
	private Point GetLargest (int[,] arr)
	{
		...
		// shouldn't happen
		if (DEBUG)
			throw new Exception ("GetLargest didn't choose anything!");
		else
	...
	private void displaySpaceState ()
	{
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				if (state [x, y] == State.UNKNOWN)
					Console.Write ("{0,3:G} ", space [x, y]);
				else
					Console.Write ("{0,3:G} ", stateChar [(int)state [x, y]]);
			}
			Console.Write ("   ");
			for (int x = 0; x < width; ++x) {
